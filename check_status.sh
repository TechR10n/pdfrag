...
curl -s http://localhost:8001/api/health | jq || echo "Failed to connect to flask-app"
...
echo "Vector DB: $(curl -s http://localhost:6333/healthz | jq -r '.status')"
echo "MLflow: $(curl -s http://localhost:5001/api/2.0/mlflow/experiments/list | jq -r '.experiments | length')"
echo "Model Server: $(curl -s http://localhost:5002/health | jq -r '.status')"
curl -s http://localhost:8001/api/health | jq || echo "Failed to connect to flask-app" 