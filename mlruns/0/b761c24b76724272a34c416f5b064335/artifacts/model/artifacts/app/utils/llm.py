...
class LLMProcessor:
    def __init__(self, model_path: str, context_size: int = 2048, max_tokens: int = 512):
        ...
        if not os.path.exists(model_path) or os.path.getsize(model_path) < 1000000:  # Less than 1MB is probably not a valid model
            logger.warning(f"Model file {model_path} not found or invalid. Using mock processor.")
            self.model = None
            self.use_mock = True
            return
