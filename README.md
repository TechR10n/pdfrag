...
**Additional Challenge:** Add a PostgreSQL container to the Docker Compose file as an alternative backend for MLflow instead of SQLite.
...
**Additional Challenge:** Create a script to benchmark the performance of the embedding model and LLM on your local hardware. Measure throughput (tokens/second) and memory usage.

## Day 2: PDF Processing and Embedding Pipeline
...
**Additional Challenge:** Enhance the PDF extraction to handle tables and preserve their structure using PyMuPDF's table extraction capabilities.
...
**Additional Challenge:** Implement a method to incrementally update the vector database when new PDFs are added, without reprocessing the entire corpus.

## Day 3: Query Processing and RAG Implementation
...
**Additional Challenge:** Implement a hybrid search that combines vector similarity with BM25 keyword matching for improved retrieval of rare terms and phrases.
...
**Additional Challenge:** Implement a document citation mechanism that links specific parts of the response to the source documents, allowing users to verify information.
...
**Additional Challenge:** Enhance the web interface with a history of past questions and answers, allowing users to revisit their previous queries.
...
**Additional Challenge:** Implement a cron job to automatically create daily backups and retain only the 7 most recent backups.
...
|-----------|--------|-------------|
| Functionality | 40% | System works as expected with all components properly integrated |
| Code Quality | 20% | Clean, well-organized, and well-documented code |
| Technical Report | 20% | Clear explanation of architecture, components, and design decisions |
| Performance | 10% | System responds in reasonable time, handles errors gracefully |
| Documentation | 10% | Comprehensive documentation for setup, usage, and troubleshooting |
...
## Day 4: MLflow Integration and Model Serving
...
**Additional Challenge:** Create a system for A/B testing different model configurations by deploying multiple versions of the model and comparing their performance.
...
**Additional Challenge:** Create a benchmark suite that evaluates the system's accuracy using a set of predefined questions with known answers.
